  ...
  (error: AxiosError<ApiErrorResponse>) => {
    ...
    if (errors && typeof errors === 'object') {
      ...
      if (firstField && Array.isArray(errors[firstField]) && errors[firstField][0]) {
        userFriendlyMessage = sanitizeErrorMessage(errors[firstField][0])
      } else if (serverMessage) {
        userFriendlyMessage = sanitizeErrorMessage(serverMessage)
      }
    } else if (status === 500) {
      ...
      userFriendlyMessage = sanitizeErrorMessage(serverMessage, 'Data yang Anda masukkan tidak valid. Silakan periksa kembali.')
    } else if (serverMessage) {
      userFriendlyMessage = sanitizeErrorMessage(serverMessage)
    }
